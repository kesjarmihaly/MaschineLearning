...
class Generator(nn.Module):
    ...
    def forward(self, x):
        out = self.conv1(x)
        out1 = self.prelu(out)
        out2 = self.res_block1(out1) + out1
        out3 = self.res_block2(out2) + out2 + out1
        out4 = self.res_block3(out3) + out3 + out2 + out1
        out5 = self.res_block4(out4) + out4 + out3 + out2 + out1
        out6 = self.res_block5(out5) + out5 + out4 + out3 + out2 + out1
        out = self.conv2(out6)
        out += out1
        out = self.up1(out)
        out = self.up2(out)
...
class GeneratorLoss(nn.Module):
    ...
    def forward(self, D_out_fake, real_img, fake_img):
        ...
        return mse_loss + 2e-8 * content_loss + self.adv_coefficient * adv_loss
