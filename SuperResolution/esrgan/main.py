...
from esrgan import Generator, Discriminator, GeneratorLoss, DiscriminatorLoss
...
def train(x_train, x_test, epochs, upscale_factor, device, pre_model, save_image_path, save_model_path):
    ...
    for epoch in range(epochs):
        ...
        for (input_img, real_img), _ in tqdm(x_train):
            ...
            D_out_real = model_D(real_img - torch.mean(fake_img, dim=0))
            D_out_fake = model_D(fake_img_tensor - torch.mean(real_img, dim=0))
        ...
        if epoch % 5 == 0 or epoch == epochs - 1:
            ...
            with torch.no_grad():
                ...
                result["test/loss_G"].append(statistics.mean(train_loss_G))
                result["test/loss_D"].append(statistics.mean(train_loss_D))
...
def arg_parser():
    ...
    parser.add_argument('--pre_model_path', type=str, default="esr_rnn/model")

    parser.add_argument('--pre_image_path', type=str, default="esr_rnn/images")

    parser.add_argument('--model_path', type=str, default="esr_gan/model")
